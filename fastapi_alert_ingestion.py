...
class AlertIngestionService:
    ...
    async def process_alerts(self, alerts: List[Dict]) -> Dict:
        ...
        try:
            ...
            noise_reduction_url = os.getenv("NOISE_REDUCTION_API_URL")
            if not noise_reduction_url:
                raise ValueError("NOISE_REDUCTION_API_URL environment variable must be set for production")
...
if __name__ == "__main__":
    ...
    # Get environment from environment variable - STRICT for production
    env = os.environ.get("ENVIRONMENT")
    if not env:
        raise ValueError("ENVIRONMENT variable must be set to either 'production' or 'development'")
    env = env.strip().lower()
    if env not in ["development", "production"]:
        raise ValueError(f"Invalid ENVIRONMENT value: {env}. Must be 'production' or 'development'")
